## Задачи
**1.** Напишете структура **Time**, която съдържа следните данни:
- две естествени числа hour и minute
- глобален operator<, който приема две времена и връща булева стойност - дали първото време е преди второто (например 9:54 е преди 11:32)

**2.** Напишете структура **Event**, която съдържа следните член-данни:
- Date (структурата от семинарите)
- Time (структурата от предната задача)
- name (символен низ с най-много 100 символа)

Реализирайте член-функция за инициализация на Event.

Напишете клас **EventCalendar**, който съдържа списък от събития (Event) (най-много 100 събития) и реализира следните действия над тях:
- printSorted - принтира събитията сортирани във възходящ ред по дата и след това по време
- addEvent(event) - добавя събитие в края на списъка
- removeEvent(index) - премахва събитието на дадения индекс
- *бонус - реализирайте функция, която проверява дали всички събития може да бъдат посетени, т.е. да няма припокриващи се събития*

**3.** Напишете **клас Teacher**, която описва преподавател. Класът трябва да съдържа:
- Име (максимум 25 символа)
- Възраст (число от 18 до 70)
- Предмет (enum с учебни предмети)
- Заплата (положително число над 800 евро)

Създайте подходящи конструктори, get и set член-функции.

**4.** Реализирайте клас/структура интервал, който представлява интервал  [a,b]  (задължително а <= b) от реалната права. Нашият интервал ще бъде дискретен - началото и краят му ще бъдат цели числа и ще разглеждаме само точки, които са цели числа. Релизирайте следният интерфейс:
...
**5.** Имплементирайте клас `Game`, който съдържа следните член-данни (полета): `title`, `price`, `isAvailable`.
