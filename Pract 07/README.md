...
- *void reverse()* - обръща реда на елементите в списъка без да заделяте допълнително памет;
...
- *U reduce(std::function<U(const T&, const U&)> operation, U null_value) const* - връща резултата от прилагането на подадената операция върху всеки един от елементите на списъка, натрупвайки го в подадената начална стойност (дясно свиване);
...
**2.** Да се напише клас `LinkedListUtils`, който поддържа следните методи за работа с обекти от класа `LinkedList` от Задача 1:
