...
Directory::Directory(const char* name, unsigned char attributes)
...
{
	if (is(Attribute::ReadOnly)) throw std::logic_error("Directory is read-only!");
...
Directory& Directory::operator+=(const FileObject& obj)
{
	if (is(Attribute::ReadOnly)) throw std::logic_error("Directory is read-only!");
	if ((*this)[obj.getName()] != nullptr) throw std::logic_error("File with such a name already exists");
	...
	data[objectsCount]->setDirectory(this);
	objectsCount++;
	return *this;
}
